...
module.exports = async function handler(req, res) {
  ...
  // Only include name if it's a real value (not empty, not "-", not a raw merge tag)
  const hasName = name && name !== '-' && !name.includes('{{');

  // Fire both requests in parallel
  const feaData = { email: email, source: 'one-click-email' };
  if (hasName) feaData.first_name = name;

  const feaPromise = fetch(FEA_CREATE_WEBHOOK, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(feaData)
